...
- **アクセシビリティ方針 (2026-05-16)**: メインターゲットはネイティブブラウザでマウス/タッチ操作するユーザー。アクセシビリティは「必要な人にも機能を提供する」最低限のサポートに留め、メインターゲットの体験 (ネイティブブラウザでの見栄え・操作性) を犠牲にしない。
  - 既存の aria-* 属性や role 属性は維持してよい (害がない範囲)
  - 新規 UI は aria-* やフォーカストラップを過剰実装せず、ネイティブ HTML 要素のデフォルト挙動に任せる
  - 確認ダイアログは `window.confirm()` または最小スタイリングの HTML `<dialog>` を優先
  - キーボードナビゲーション・スクリーンリーダー最適化のために UnoCSS の見栄えや UX を曲げない
