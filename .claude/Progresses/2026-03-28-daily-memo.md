...
## タスク: Plan 0005 — デイリースクラムメモ欄

### Phase A: 基盤
- [x] Markdown エディタライブラリ選定・インストール（Tiptap）
- [x] DailyMemo 型定義（`src/types/`）
- [x] useMemo composable 新規作成（CRUD、localStorage永続化、日別保存）
- [x] useMemo テスト作成（11テスト）

### Phase B: UI
- [x] MemoEditor.vue 新規作成（公開/秘匿タブ切り替え、Tiptap WYSIWYG）
- [x] PublicMemoList.vue 新規作成（みんなの公開メモ表示、折りたたみ可能）
- [x] TimerView.vue を2カラムレイアウトに改修（md:grid-cols-2）

### Phase C: WebRTC同期
- [x] RoomMessage に memo-update 追加
- [x] useRoom にメモ同期（デバウンス500msブロードキャスト）追加

### 品質ゲート
- [x] Stage 1: `vp build && vp check src/ && vp test run` — 61テスト全パス
- [x] Stage 2: レビュー（Critical XSS修正済み、Warning 3件修正済み）・コントリビューション（候補なし）
- [x] 完了処理
