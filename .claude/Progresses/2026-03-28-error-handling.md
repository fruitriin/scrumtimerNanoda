...
### ノウハウ記録（3つのタイミング）
実装中・レビュー後・タスク総括時に、以下のカテゴリで知見を `/addf-knowhow` で記録する:

| タイミング | 記録内容 | 例 |
|---|---|---|
| **実装中** | コーディングパターン、ライブラリの癖、ハマりポイント | `vue-coding-patterns.md` |
| **レビュー後** | 品質ゲートエージェントが検出した問題パターン、運用上の気づき | `quality-gate-patterns.md` |
| **タスク総括時** | タスク選択判断、時間配分、プロセス改善 | `task-completion-patterns.md` |
...
5. `addf-code-review-agent` と `addf-contribution-agent` を**並列**（`run_in_background: true`）で起動する
6. レビュー指摘への対応:
...
## タスク: Plan 0015 — エラーハンドリング強化

- [ ] useRoom: 再接続/マイグレーション中の errorMessage クリア
- [ ] useMemo: localStorage quota exceeded 対応
- [ ] useParticipants/useSettings: 同様の quota 対応
- [ ] useAudio: 再生失敗の UI 通知
- [ ] Stage 1
- [ ] Stage 2
- [ ] 完了処理
