# 計画: VoiceVox 音源タイムキープ — 完了 (2026-03-23)

> **実装方式変更**: VoiceVox Engine リアルタイム合成ではなく、プリベイクWAV方式で実装。
> VoiceVox スキルで事前に音声ファイルを生成し、`public/` に配置。
> `useAudio` composable がトラック定義から一括管理し、`useTimer` の tick で再生タイミングを制御。
> セリフはオーナー指定のものに変更（下記の読み上げルール表とは異なる）。
