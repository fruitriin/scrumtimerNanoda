...
> メインターゲットはネイティブブラウザでマウス/タッチ操作するユーザー。
> アクセシビリティは「必要な人にも機能を提供する」最低限のサポートに留め、
> メインターゲットの体験を犠牲にしない。
> （`.claude/Feedback.md` オーナーフィードバック参照）
...
ネイティブブラウザの操作性を上げるキーボード関連の実装は **全員に価値のある快適さ要件**。
...
- **NG パターン**: `<button title="不在にする">⊖</button>` だけで意味を伝える
  ...
  - これは致命的なネイティブ体験の損失。`title` 単独に頼った設計は不可
